  ...
  return <div className="relative flex-1">
      ...
      msOverflowStyle: 'none'
        ...
        {primaryCommands.map(command => <TooltipProvider key={command.id}>
            <Tooltip>
              <TooltipTrigger asChild>
                <Button size="sm" onClick={() => handleCommandClick(command)} disabled={isLoading} className="flex items-center gap-1.5 whitespace-nowrap min-w-fit h-11 bg-command-button text-command-button-foreground hover:bg-command-button/90 border-0 transition-all duration-200 rounded-sm text-gray-950 bg-zinc-300 hover:bg-zinc-200 text-sm px-[10px] py-[10px] mx-[5px] my-[5px]">
