...
import { X, Send, Bot, User, RotateCcw, FileText } from 'lucide-react';
  ...
  const handleSourceClick = async (sourceId: string) => {
    try {
      const {
        data: doc,
        error
      } = await supabase.from('documents').select('*').eq('id', sourceId).single();
      if (error) throw error;
      if (doc) {
  ...
  return <div className="w-full h-full flex flex-col bg-sidebar px-[40px]">
      ...
      <div className="flex-shrink-0 flex items-center justify-between p-5 border-b border-sidebar-border bg-sidebar shadow-sm py-0">
        ...
        <div className="flex items-center gap-2">
          <Button variant="ghost" size="sm" onClick={clearChat} disabled={messages.length === 0} className="text-muted-foreground hover:text-sidebar-foreground hover:bg-sidebar-accent">
            <RotateCcw className="w-4 h-4 mr-2" />
            New Chat
          </Button>
          <Button variant="ghost" size="icon" onClick={onClose} className="text-muted-foreground hover:text-sidebar-foreground hover:bg-sidebar-accent">
            <X className="w-4 h-4" />
          </Button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-hidden bg-surface/30">
        <ScrollArea className="h-full">
          <div className="p-5">
            {messages.length === 0 ? <div className="flex flex-col items-center justify-center h-full min-h-[400px] text-center">
                ...
                {messages.map(message => <div key={message.id} className="animate-fade-in">
                    ...
                    <div className="flex items-center gap-3 mb-3">
                      <div className={`p-1.5 rounded-lg ${message.role === 'user' ? 'bg-sidebar-primary/10 border border-sidebar-primary/20' : 'bg-muted/50 border border-border'}`}>
                        {message.role === 'user' ? <User className="w-4 h-4 text-sidebar-primary" /> : <Bot className="w-4 h-4 text-muted-foreground" />}
                      </div>
                      <span className={`text-body-sm font-medium ${message.role === 'user' ? 'text-sidebar-primary' : 'text-muted-foreground'}`}>
                        {message.role === 'user' ? 'You' : 'Assistant'}
                      </span>
                    </div>
                    
                    {/* Enhanced Message Content */}
                    <div className={`ml-10 p-4 rounded-xl border transition-all duration-200 ${message.role === 'user' ? 'bg-sidebar-primary text-sidebar-primary-foreground border-sidebar-primary/20 shadow-sm' : 'bg-card border-border shadow-sm hover:shadow-md'}`}>
                      <div className="text-body-md leading-relaxed whitespace-pre-wrap">
                        {message.content}
                      </div>
                      
                      {/* Enhanced Sources */}
                      {message.sources && message.sources.length > 0 && <div className="mt-4 pt-4 border-t border-border/20">
                          <span className="text-caption text-muted-foreground font-medium">Referenced documents:</span>
                          <div className="flex flex-wrap gap-2 mt-2">
                            {message.sources.map(source => <button key={source.id} onClick={() => handleSourceClick(source.id)} className="inline-flex items-center gap-2 px-3 py-1.5 text-xs bg-muted/50 hover:bg-sidebar-primary hover:text-sidebar-primary-foreground rounded-lg transition-all duration-200 border border-border hover:border-sidebar-primary/20 shadow-sm hover:shadow-md">
                ...
                {isLoading && <div className="animate-fade-in">
                    <div className="flex items-center gap-3 mb-3">
